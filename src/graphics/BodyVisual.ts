...
function newCelestialVisual(scene: Scene, lt: Lifetime, spatial: Spatial) {
  ...
  obj.property('size', Number).getThen(lt, km => {
...
function newShipVisual(scene: Scene, lt: Lifetime, spatial: Spatial) {
  const obj = spatial.bodyObj();
  const direction = new THREE.Vector3();
  let accel: Vec3 | undefined;
  obj.property('accel', Vec3).subscribe(lt, newAccel => {
    accel = newAccel;
  });
...
export function newBodyVisual(scene: Scene, game: Game, obj: SsObject) {
  ...
  obj.property('class', String).getThen(lt, cls => {
    if (cls === 'celestial') {
      newCelestialVisual(scene, lt, spatial);
    } else if (cls === 'ship') {
      newShipVisual(scene, lt, spatial);
    } else {
